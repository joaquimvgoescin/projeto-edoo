...
int dimencao_mapax = 10;

int dimencao_mapay = 10;
    ...
    for(int i = 1; i <= dimencao_mapay; i++){
        ...
        for (int j = 1; j <= dimencao_mapax; j++){
            ...
            if (numero_aleatorio == 1){
                mapa[i - 1].push_back(pedra(ouro(j * 10,i * 10)));
            }

            else if (numero_aleatorio == 2){
                mapa[i - 1].push_back(pedra(prata(j * 10,i * 10)));
            }

            else if (numero_aleatorio == 2){
                mapa[i - 1].push_back(pedra(cobre(j * 10,i * 10)));
            }

            else{
                mapa[i - 1].push_back(pedra(terra(j * 10,i * 10)));
            }
